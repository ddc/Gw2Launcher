#! /usr/bin/env python3
#|*****************************************************
# * Copyright         : Copyright (C) 2018
# * Author            : Hadesz#1223 (discord)
# * E-Mail            : hadesz456@gmail.com
# * License           : GPL v3
# * Python            : 3.5
#|*****************************************************
# # -*- coding: utf-8 -*-

find_gw2_exec                   = "Please find GW2 executable file before continue!!!"

not_valid_gw2                   = "is not a valid Gw2 executable!!!\nThis program needs a valid GW2 file."

arcdps_404                      = "Could not find arcdps file to download from the website!!!!"

arcdps_timeout                  = "Unable to reach Arcdps website to check for updates!!!"

arcdps_new_version              = "Checking for new version of ArcDps: "

arcdps_updating                 = "Updating ArcDps: "

arcdps_installing               = "Installing ArcDps: "

arcdps_error_dl                 = "Error downloading arcdps!!!"

arcdps_disclamer                = "I did not write this loader nor do i provide any support for it or it's compatibility with Guild Wars 2.\n"\
                                + "Credit goes to /u/deltaconnected for the code."
                        
dl_new_version                  = "Downloading New Version..."

error_dl_new_version            = "Theres was an ERROR trying to download new version.\nPlease try again later"

info_dl_new_version             = "Download completed sucessfully.\nNew version downloaded to \"Downloads\" directory."

new_version_available           = "Theres a new version available!!!"

check_downloaded_dir            = "If have you already downloaded,\nplease check your download directory!!!"

confirm_download                = "Do you want to download now?"

remote_version_file_not_found   = "ERROR: Remote File: VERSION was not found."

need_find_gw2                   = "<== NEED TO FIND Gw2-64.exe TO ENABLE ALL PROGRAM FUNCTIONALITIES!!!"
